...
{
    // The historical custom Gmsh writer API was removed from Foundation 13.
    // Signal sampling remains active; geometry is handled by native sampledSurface
    // writers or by the post-processing pipeline.
    Info<< "Skipping legacy Gmsh geometry writer in Foundation 13 port" << nl;
}
