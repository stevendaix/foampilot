...
static List<scalar> degToRadList(const List<scalar>& values)
{
    List<scalar> converted(values.size());
    forAll(values, i)
    {
...
Foam::List<Foam::scalar> Foam::fv::LeishmanBeddoes::cnToF
(
    List<scalar> cnList,
...
Foam::scalar Foam::fv::LeishmanBeddoes::interpolateStaticParam(word paramName)
{
...
)
{
    ...
    List<scalar> alphaList = degToRadList
    (
...
{
    scalar pi = Foam::constant::mathematical::pi;
    List<scalar> alpha = degToRadList(profileData_.angleOfAttackList(0.5, 25));
