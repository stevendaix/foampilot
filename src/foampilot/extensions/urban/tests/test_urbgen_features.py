...
def test_multi_site_api_returns_deterministic_keyed_results():
    ...
    from foampilot.extensions.urban.generation import generate_urbgen_multi_site
...
def test_max_feasible_boundary_move_preserves_containment():
    from foampilot.extensions.urban.generation.urbgen import _move_to_boundary
...
def test_max_feasible_podium_move_preserves_containment():
    from foampilot.extensions.urban.generation.urbgen import _move_tower_to_podium_edge
    region = SITE.buffer(-8)
    tower = Polygon([(60, 50), (72, 50), (72, 62), (60, 62)])
...
def test_ringcache_arc_mapping_and_disjoint_segments():
    from foampilot.extensions.urban.generation.urbgen import RingCache, _courtyard_ring_segments
    zone = SITE.buffer(-8)
...
def test_typology_primitives_are_explicit_and_area_consistent():
    from foampilot.extensions.urban.generation.urbgen import (
    ...
    for mode in range(6):
        ...
        assert all(len(module) == 4 for module in modules)
    ...
    assert get_typology_modules(24.0, 8.0, 1, 8.0)[1] == (-12.0, 2.0, 8.0, 12.0)
    assert get_typology_modules(24.0, 8.0, 2, 8.0)[1] == (-12.0, 0.0, 8.0, 24.0)
...
def test_populate_region_modes_holes_rotation_and_seed():
    from foampilot.extensions.urban.generation import PopulateRegionConfig, populate_region
