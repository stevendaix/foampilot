#!/usr/bin/env python3
"""
Run the Wolf Dynamics damBreak VOF OpenFOAM 13 tutorial via FoamPilot.
"""
...
from adapter import DamBreakVOFTutorial
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to the extracted damBreak VOF case.",
    )
    parser.add_argument(
        "--run-root", type=Path, default=Path(".runs/dambreak"),
        help="Disposable directory where the generated run case is written.",
    )
    args = parser.parse_args()

    target = args.run_root / "damBreak"
    tutorial = DamBreakVOFTutorial(
