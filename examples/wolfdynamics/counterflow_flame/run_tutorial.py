#!/usr/bin/env python3
"""Run the Figshare CounterFlow Flame tutorial entirely through FoamPilot.

The script never invokes an ``Allrun`` script or ``foamRun`` itself.  FoamPilot
writes the final input dictionaries, runs the mesh check through ``BaseSolver``
and launches the calculation with its standard ``run_simulation`` API.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-root", type=Path, default=ROOT / ".runs/counterflow_flame")
    parser.add_argument("--end-time", type=float, default=20.0)
    parser.add_argument("--write-interval", type=float, default=10.0)
...
def write_foampilot_inputs(source: Path, target: Path, end_time: float, write_interval: float) -> None:
    """Copy assets then write every text OpenFOAM input with FoamPilot."""
    if target.exists():
...
def main() -> None:
    ...
    target = (args.run_root / "counterFlowFlame2DLTS-OF13Units").resolve()
    write_foampilot_inputs(source, target, args.end_time, args.write_interval)

    # The public FoamPilot solver API owns the execution commands and logs.
    ...
    print(f"FoamPilot completed CounterFlow Flame: {target}")
