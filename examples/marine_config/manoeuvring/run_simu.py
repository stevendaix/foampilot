#!/usr/bin/env python3
"""Free-running turning manoeuvre — OpenFOAM 13 VoF + dynamic mesh.

Recrée un cas de manoeuvre de virement en utilisant les helpers généraux
de foampilot, à l'image de ``dtc_moving/run_simu.py``.
"""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manoeuvring turning case.")
    parser.add_argument("--processors", type=int, default=8, help="MPI ranks.")
...
def build_case() -> Path:
    case_path = CASE_ROOT / "manoeuvring_turning"
    if case_path.exists():
    ...
    write_dynamic_mesh_dict(
        ...
        patch_name="hull",
        mass=412.73,
        centre_of_mass=(2.929541, 0.0, 0.2),
        inertia=(40.0, 0.0, 0.0, 921.0, 0.0, 921.0),
        transform_origin=(0.0, 0.0, 0.0),
        joints=("Pz", "Ry"),
        inner_distance=0.3,
        outer_distance=1.0,
        translation_damper_coeff=8596,
        rotation_damper_coeff=11586,
    )
...
""")
    ...
    solver.system.write_functions_file(system_path, rigid_body=True)
...
castellatedMeshControls
{
    ...
    // Note: this is the number of cells before removing the part which
