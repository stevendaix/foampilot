#!/usr/bin/env python3
"""
Visualisation 2D corrigée : chaque méthode ne montre que ses propres faces inlet/outlet,
avec un fond gris pour toutes les faces de bord.
"""
import sys
...
TIME_STEP = "500"
OUT_DIR = Path(__file__).resolve().parent / "method_images_2d_v2"
...
def assign_faces_to_caps(reader, cap1_center, cap2_center, axis, centroids, max_faces_per_cap=500):
    points = reader._points
    faces = reader._faces
    patches = reader.boundary_patches
    centers = []
    ...
    # Only assign faces that are clearly closer to one cap
    # Use a ratio threshold
    ratio = d1 / (d1 + d2 + 1e-12)
    cap1_idx = np.where(ratio < 0.3)[0]
    cap2_idx = np.where(ratio > 0.7)[0]
    
    # Limit to max_faces_per_cap
...
def save_method_image_2d(all_centers_2d, boundary_ids, inlet_faces, outlet_faces, title, out_path):
    ...
    # Fond gris clair pour TOUTES les faces de bord
...
def save_p02_image_2d(all_centers_2d, boundary_ids, inlet_pt, outlet_pt, title, out_path):
    ...
    # Fond gris pour toutes les faces de bord
    boundary_idx = np.array(sorted(boundary_ids))
    boundary_idx = boundary_idx[boundary_idx < len(all_centers_2d)]
    if len(boundary_idx) > 0:
        ax.scatter(
...
def main():
    ...
    # Collect all boundary face centers and IDs
    boundary_centers = []
    boundary_ids = []
    for name, info in patches.items():
        sf, nf = info.get("startFace", 0), info.get("nFaces", 0)
    ...
    p04_inlet, p04_outlet = assign_faces_to_caps(reader, p04["inlet"], p04["outlet"], axis, centroids, max_faces_per_cap=500)
    save_method_image_2d(
        all_centers_2d,
        boundary_ids,
        p04_inlet,
    ...
    p23_inlet, p23_outlet = assign_faces_to_caps(reader, p23["inlet"], p23["outlet"], axis, centroids, max_faces_per_cap=500)
    save_method_image_2d(
        all_centers_2d,
        boundary_ids,
        p23_inlet,
    ...
    p24_inlet, p24_outlet = assign_faces_to_caps(reader, p24["inlet"], p24["outlet"], axis, centroids, max_faces_per_cap=500)
    save_method_image_2d(
        all_centers_2d,
        boundary_ids,
        p24_inlet,
