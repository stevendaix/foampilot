#!/usr/bin/env python3
"""
Visualisation 2D par PCA des centres de faces OpenFOAM.
Chaque méthode = scatter plot 2D avec inlet/outlet bien visible.
"""
import sys
...
TIME_STEP = "500"
OUT_DIR = Path(__file__).resolve().parent / "method_images_2d"
...
def save_method_image_2d(all_centers_2d, inlet_faces, outlet_faces, title, out_path):
    ...
    # Fond gris clair pour toutes les faces
    ax.scatter(
        all_centers_2d[:, 0],
        all_centers_2d[:, 1],
        c="#cccccc",
    ...
    if len(inlet_idx) > 0 and inlet_idx.max() < len(all_centers_2d):
        ax.scatter(
    ...
    if len(outlet_idx) > 0 and outlet_idx.max() < len(all_centers_2d):
        ax.scatter(
...
def save_p02_image_2d(all_centers_2d, inlet_pt, outlet_pt, title, out_path):
    fig, ax = plt.subplots(1, 1, figsize=(10, 8), facecolor="white")
    ax.set_facecolor("white")
    ax.set_title(title, fontsize=14, pad=12)
    ax.set_xlabel("PC1")
    ...
    # Project inlet/outlet points onto 2D PCA space
    # Use the same PCA fitted on all_centers
...
def main():
    ...
    # Calculer les centres de faces OpenFOAM
    face_centers = []
    for fi in range(len(faces)):
        face_centers.append(points[faces[fi]].mean(axis=0))
    face_centers = np.array(face_centers)

    # 2D projection des centres de faces
    global pca2d
    pca2d = PCA(n_components=2)
    all_centers_2d = pca2d.fit_transform(face_centers)
    print("face_centers shape:", face_centers.shape)
    print("all_centers_2d shape:", all_centers_2d.shape)
