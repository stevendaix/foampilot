...
def main():
    ...
    p11_inlet_ids = set(p11["inlet_faces"])
    p11_outlet_ids = set(p11["outlet_faces"])
    ...
    # P11 centers
    p11_inlet_center = np.mean([points[faces[fi]].mean(axis=0) for fi in p11_inlet_ids], axis=0) if p11_inlet_ids else np.array([0, 0, 0])
    p11_outlet_center = np.mean([points[faces[fi]].mean(axis=0) for fi in p11_outlet_ids], axis=0) if p11_outlet_ids else np.array([0, 0, 0])

    # Face area function
    def face_area(face_id):
        face = faces[face_id]
        if len(face) < 3:
            return 0.0
        pts = points[face]
    ...
    methods = {
        "P02_inlet": {"faces": [], "center": p02_inlet},
        "P02_outlet": {"faces": [], "center": p02_outlet},
        "P04_inlet": {"faces": p04_inlet_faces, "center": p04["inlet"]},
        "P04_outlet": {"faces": p04_outlet_faces, "center": p04["outlet"]},
        "P11_inlet": {"faces": p11_inlet_ids, "center": p11_inlet_center},
        "P11_outlet": {"faces": p11_outlet_ids, "center": p11_outlet_center},
    ...
    for name, data in methods.items():
        ...
        center = data["center"]
        if face_list:
            areas = [face_area(fi) for fi in face_list]
            total_area = sum(areas)
            mean_area = np.mean(areas)
            face_centers = np.array([points[faces[fi]].mean(axis=0) for fi in face_list])
            ...
            print(f"\n{name}:")
            print(f"  Centre: [{center[0]:.4f}, {center[1]:.4f}, {center[2]:.4f}]")
            print(f"  (pas de faces, extrémités centerline)")
    ...
    for name, data in methods.items():
        if "P02" in name or not data["faces"]:
            continue
        face_list = list(data["faces"])
        face_centers = np.array([points[faces[fi]].mean(axis=0) for fi in face_list])
        center = data["center"]
    ...
    for prefix in ["P04", "P11", "P13"]:
        ...
        if inlet_key in methods and outlet_key in methods:
            ...
            inlet_area = sum(face_area(fi) for fi in inlet_faces)
            outlet_area = sum(face_area(fi) for fi in outlet_faces)
