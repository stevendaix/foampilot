#!/usr/bin/env python3
"""
Visualisation des méthodes fiables P02, P04, P11, P13, P18, P23, P24
Entrée en bleu, sortie en rouge, reste en gris.
"""
import sys
...
TIME_STEP = "500"
OUT_DIR = Path(__file__).resolve().parent / "method_images"
...
def save_method_image(mesh, inlet_faces, outlet_faces, title, out_path):
    ...
    plotter.add_mesh(mesh, color="lightgray", show_edges=False, opacity=0.3)

    cell_colors = np.full(mesh.n_cells, "lightgray")
    for fi in inlet_faces:
        if 0 <= fi < mesh.n_cells:
            cell_colors[fi] = "blue"
    ...
    colored = mesh.copy()
    colored.cell_data["color"] = cell_colors

    plotter.add_mesh(colored, scalars="color", show_edges=False, opacity=0.9)
...
def main():
    ...
    save_method_image(
        ...
        "P11 — Patches OpenFOAM (INLET=376, OUTLET=2512)",
    ...
    for name, func in [
        ...
        save_method_image(
            ...
            inlet_faces,
            outlet_faces,
            f"{name} — Caps plans ({len(inlet_faces)} inlet, {len(outlet_faces)} outlet)",
    ...
    plotter = pv.Plotter(off_screen=True, window_size=(1200, 800))
    plotter.add_mesh(mesh, color="lightgray", show_edges=False, opacity=0.3)
    plotter.add_mesh(pv.Sphere(radius=0.001, center=p02["inlet"]), color="blue", label="Inlet")
    plotter.add_mesh(pv.Sphere(radius=0.001, center=p02["outlet"]), color="red", label="Outlet")
    plotter.add_title("P02 — Centerline PCA (extrémités)", font_size=12)
    plotter.screenshot(str(OUT_DIR / "P02_inlet_outlet.png"))
    plotter.close()
    ...
    save_method_image(
        ...
        opening0_faces,
        opening1_faces,
        f"P13 — Graphe topologique ({len(opening0_faces)} opening0, {len(opening1_faces)} opening1)",
    ...
    save_method_image(
        ...
        inlet_faces,
        outlet_faces,
        f"P18 — Convention s_min/s_max ({len(inlet_faces)} inlet, {len(outlet_faces)} outlet)",
