#!/usr/bin/env python3
"""
Fixed 2D scatter visualization for inlet/outlet detection methods.
Uses matplotlib only, explicit face_id -> boundary_index mapping.
"""
import sys
...
def method_p02_centerline(reader, mesh):
    ...
    end1 = centroids[np.argmin(s)]
    end2 = centroids[np.argmax(s)]
...
def method_p11_openfoam_patches(reader, mesh):
    ...
    return {"inlet_faces": inlet_ids, "outlet_faces": outlet_ids, "axis": axis}
...
def method_p13_topological_graph(reader, mesh):
    ...
    if len(terminal_centers) >= 2:
        ...
        opening0_center = opening1_center = terminal_centers[0] if len(terminal_centers) > 0 else np.array([0.0, 0.0, 0.0])
...
def method_p23_priority_tests(reader, mesh):
    ...
    if s1 < s2:
        ...
        inlet_center, outlet_center = cap2_center, cap1_center
    return {"inlet": inlet_center, "outlet": outlet_center, "n_caps": 2, "axis": axis}
