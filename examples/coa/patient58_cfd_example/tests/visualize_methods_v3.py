...
Entrée en bleu, sortie en rouge, contours clairs.
"""
import sys
...
TIME_STEP = "500"
OUT_DIR = Path(__file__).resolve().parent / "method_images_v3"
...
def save_method_image(mesh, inlet_faces, outlet_faces, title, out_path):
    ...
    # Sous-échantillonnage pour la visibilité
    max_cells = 4000
    if mesh.n_cells > max_cells:
        rng = np.random.default_rng(0)
        idx = rng.choice(mesh.n_cells, size=max_cells, replace=False)
        idx = np.sort(idx)
    else:
    ...
    ax.scatter(
        mesh.cell_centers().points[idx, 0],
        mesh.cell_centers().points[idx, 1],
        mesh.cell_centers().points[idx, 2],
        c="#dddddd",
        s=0.4,
        alpha=0.35,
        marker="o",
    ...
    if len(inlet_faces) > 0:
        ...
        inlet_idx = inlet_idx[inlet_idx < mesh.n_cells]
        if len(inlet_idx) > 0:
            ax.scatter(
                mesh.cell_centers().points[inlet_idx, 0],
                mesh.cell_centers().points[inlet_idx, 1],
                mesh.cell_centers().points[inlet_idx, 2],
                c="#0066cc",
                s=6.0,
                alpha=0.9,
                marker="o",
    ...
    if len(outlet_faces) > 0:
        ...
        outlet_idx = outlet_idx[outlet_idx < mesh.n_cells]
        if len(outlet_idx) > 0:
            ax.scatter(
                mesh.cell_centers().points[outlet_idx, 0],
                mesh.cell_centers().points[outlet_idx, 1],
                mesh.cell_centers().points[outlet_idx, 2],
                c="#cc2200",
                s=6.0,
                alpha=0.9,
                marker="o",
    ...
    # Égale échelle
    pts = mesh.cell_centers().points[idx]
