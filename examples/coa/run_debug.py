...
def setup_coa_case():
    ...
    stl_src_path = current_path / "data"
    ...
    for stl_file in stl_src_path.glob("*.stl"):
    ...
    snappy = mesh.mesher
    snappy.stl_file = stl_dest / "wall_aorta.stl"

    
    # Configure snappyHexMeshDict
    snappy.locationInMesh = (-16.3177, -21.6838, -12.3357)

    analyzer = STLAnalyzer(stl_dest / "wall_aorta.stl")
    analyzer.load()
    snappy.locationInMesh = analyzer.get_center_of_mass()



    print(f"Center of Mass of the STL: {analyzer.get_center_of_mass()}")
    ...
    stl_files_for_emesh = ["wall_aorta.stl", "inlet.stl", "outlet1.stl", "outlet2.stl", "outlet3.stl", "outlet4.stl"]
    snappy.write_surface_features_dict(stl_list_for_emesh=stl_files_for_emesh, included_angle=30)

    snappy.write_block_mesh_dict()
