...
class OCCBuilder:
    ...
    def _sweep_along_centerline(self, sections: List[Section], curves: List[np.ndarray], name: str) -> int:
        ...
        This is more robust than lofting when section contours have
        minor self-intersections or orientation issues.

        Args:
            sections: List of Section objects.
            curves: List of 3D curve points (one per section).
        ...
        # Use a circular profile as fallback if the contour is irregular
        if len(pts2d) < 3:
            raise RuntimeError("First section has too few points for profile")

        # Create the profile as a BSpline curve in the section's local plane
    ...
    def _create_path_curve(self, path_points: np.ndarray) -> int:
        ...
        # Resample to reduce number of control points for smoother curve
