    ...
    for (int k=0; k < nz; k++)
    {
      for (int j=0; j < ny; j++)
      {
        for (int i=0; i < nx; i++)
        {
          ...
          cellIndex = ms.findCell(ptemp); // faster
...
int main(int argc, char *argv[])
{
    ...
    scalar kc = coeffs.lookupOrDefault<scalar>("kc", 0.5);
    scalar minCellSizeFactor = coeffs.lookupOrDefault<scalar>("minCellSizeFactor", 10); 
