...
def main():
 ...
 if args.closing > 0 or args.erosion > 0:
  from scipy import ndimage
  if args.closing > 0: occ=ndimage.binary_closing(occ, iterations=args.closing)
  if args.erosion > 0: occ=ndimage.binary_erosion(occ, iterations=args.erosion)
 ...
 report={'inputs':[x[0] for x in grids],'pitch':args.pitch,'closing':args.closing,'erosion':args.erosion,'grid_shape':list(map(int,shape)),'occupied_voxels':int(occ.sum()),'vertices':int(len(mesh.vertices)),'faces':int(len(mesh.faces)),'watertight':bool(mesh.is_watertight),'winding_consistent':bool(mesh.is_winding_consistent),'components':len(mesh.split(only_watertight=False)) if hasattr(mesh,'split') else None,'volume':float(mesh.volume),'elapsed_seconds':round(time.perf_counter()-t0,6)}; args.output.with_suffix('.json').write_text(json.dumps(report,indent=2)); print(json.dumps(report,indent=2))
