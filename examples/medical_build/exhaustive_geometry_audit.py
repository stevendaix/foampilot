...
root=Path(__file__).resolve().parents[2]; out=root/'examples/medical_build/outputs'; out.mkdir(exist_ok=True)
files=[
 root/'foampilot/test/vmtk_test_data/aorta-surface.vtp',
 root/'foampilot/test/vmtk_test_data/aorta-surface-branch-split.vtp',
 Path(__file__).resolve().parents[2] / 'case_complex/openfoam_surface_patches/aorta_surface_patches.vtp',
 ...
 Path(__file__).resolve().parents[2] / 'case_complex/openfoam_surface_patches/case_motorbike_style/constant/triSurface/wall.stl',
]
def audit(p):
