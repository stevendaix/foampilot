"""Tobias Holzmann arbitrary water pump tutorial for OpenFOAM 13."""
...
def write_case() -> None:
    if CASE.exists():
        shutil.rmtree(CASE)
    writer = OpenFOAMDictAddFile(object_name="tobias_arbitrary_water_pump")
...
def run() -> None:
    ...
    if not background_mesh.is_file():
        raise FileNotFoundError(
            "arbitrary_water_pump requires cad/backgroundMesh.unv. "
            "The large Tobias asset is not stored in GitHub; download the "
            "complete case archive from Holzmann CFD before running OpenFOAM 13."
        )
    ...
    print(f"Validated arbitrary water pump workflow: {CASE}")
