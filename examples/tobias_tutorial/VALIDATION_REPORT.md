...
| Catalyst Heat-Up | `ideasUnvToFoam` → `snappyHexMesh` → `splitMeshRegions -cellZones` → regional `createPatch` → `foamMultiRun` | Three regions (`fluid`, `SCR1`, `SCR2`) created; inter-region heat-transfer models loaded; 48,144 and 48,146 cell couplings calculated; first thermo-fluid time step converged and reached `End` | Validated short multi-region CHT calculation |
