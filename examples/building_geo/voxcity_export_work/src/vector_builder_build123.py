#!/usr/bin/env python3
"""
Vector Gmsh builder: build OpenFOAM-ready single-region mesh
from UrbanModel + CFDTerrain using native Gmsh geometry.
"""
import sys
...
class VectorGmshBuilder:
    ...
    def build(self, margin: float = None, bottom_offset: float = 5.0):
        """Build fluid domain and building volumes using native Gmsh geometry.
        ...
        print("  Building geometry with Gmsh OCC...")
        dx = domain_xmax - domain_xmin
        dy = domain_ymax - domain_ymin
        dz = domain_zmax - domain_zmin

        fluid_tag = gmsh.model.occ.addBox(domain_xmin, domain_ymin, domain_zmin, dx, dy, dz)
        gmsh.model.occ.synchronize()
    ...
    def export_openfoam(self, case_path: Path):
        ...
        self._inject_buildings_patch(case_path)
    ...
    def _inject_buildings_patch(self, case_path: Path):
        boundary_path = case_path / "constant" / "polyMesh" / "boundary"
        if not boundary_path.exists():
            print("DEBUG inject: boundary missing")
            return
        patch_to_surfaces = getattr(self, "_patch_to_surfaces", None)
        if not patch_to_surfaces or "buildings" not in patch_to_surfaces:
            print(f"DEBUG inject: patch_to_surfaces={patch_to_surfaces}")
            return
        text = boundary_path.read_text()
        if "buildings" in text:
            print("DEBUG inject: already present")
            return
        n_building_faces = len(patch_to_surfaces.get("buildings", []))
        print(f"DEBUG inject: n_building_faces={n_building_faces}")
        if n_building_faces <= 0:
            return
        ...
        for idx, line in enumerate(lines):
            stripped = line.strip()
            if stripped.isdigit():
                n_patches = int(stripped)
                patch_start = idx + 1
                break
        print(f"DEBUG inject: n_patches={n_patches}, patch_start={patch_start}")
        if n_patches is None or patch_start is None:
            return
        start = patch_start
        for _ in range(n_patches):
            while start < len(lines) and lines[start].strip() != ")":
                start += 1
            start += 1
        print(f"DEBUG inject: start={start}, len(lines)={len(lines)}")
        counts = self._extract_face_counts(lines[patch_start:start])
        print(f"DEBUG inject: counts={counts}, total_faces={sum(counts) if counts else 0}")
        total_faces = sum(counts) if counts else 0
        insert = [
            "    buildings",
            "    {",
            "        type            wall;",
            f"        nFaces           {n_building_faces};",
            f"        startFace        {total_faces + 1};",
            "    }",
            "",
        ]
        new_lines = lines[:start] + insert + lines[start:-3] + lines[-3:]
        boundary_path.write_text("\n".join(new_lines) + "\n")
        print(f"  Injected buildings patch with {n_building_faces} faces")

    def _extract_face_counts(self, lines):
        counts = []
        i = 0
        while i < len(lines):
            line = lines[i].strip()
            if line.endswith("{"):
                ...
                while i < len(lines) and lines[i].strip() != "}":
                    if lines[i].strip().startswith("nFaces"):
                        try:
                            counts.append(int(lines[i].split()[1].rstrip(";")))
                        except Exception:
                            pass
                    i += 1
            i += 1
