...
# Merge nearby buildings: buffer(0.5) then buffer(-0.5)
merged_polys = unary_union(clean_polys)
merged_polys = make_valid(merged_polys)
merged_polys = merged_polys.buffer(0.0)
# Merge nearby buildings with distance=1.0
merged_polys = unary_union(clean_polys)
merged_polys = make_valid(merged_polys)
merged_polys = merged_polys.buffer(0.0)
# Merge nearby buildings with distance=1.0
merged_polys = unary_union([p.buffer(1.0) for p in (merged_polys.geoms if merged_polys.geom_type == "MultiPolygon" else [merged_polys])])
merged_polys = make_valid(merged_polys)
merged_polys = merged_polys.buffer(0.0)
# Close small gaps between merged buildings: buffer(+gap/2) then buffer(-gap/2)
gap = 0.5
merged_polys = unary_union([p.buffer(gap / 2.0) for p in (merged_polys.geoms if merged_polys.geom_type == "MultiPolygon" else [merged_polys])])
merged_polys = make_valid(merged_polys)
merged_polys = merged_polys.buffer(0.0)
merged_polys = unary_union([p.buffer(-gap / 2.0) for p in (merged_polys.geoms if merged_polys.geom_type == "MultiPolygon" else [merged_polys])])
merged_polys = make_valid(merged_polys)
...
# If MultiPolygon, keep individual merged polygons
if merged_polys.geom_type == "MultiPolygon":
    merged_list = [p for p in merged_polys.geoms if not p.is_empty]
else:
    merged_list = [merged_polys] if not merged_polys.is_empty else []

print(f"Raw: {len(raw_polys)}, Cleaned: {len(clean_polys)}, Merged: {len(merged_list)}")
...
plot_polys(axes[2], merged_list, f"Merged nearby buildings: {len(merged_list)}", "#f58518")

fig.suptitle("VoxCity footprint processing — EPSG:32631", fontsize=14)
fig.tight_layout()
out = Path("footprint_processing_steps.png")
