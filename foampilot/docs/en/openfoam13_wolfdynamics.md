...
**Phase B — minimal teaching cases.** Port the cavity, Hagen–Poiseuille, cylinder and dam-break cases first. Each case must be generated by FoamPilot, inspectable before execution and accompanied by a compact report covering files, mesh, residuals, conservation, reference quantity and tolerance.
...
**Phase D — OF13 chemistry and combustion.** The October 2025 Figshare edition explicitly announces OpenFOAM 13 and provides examples and material on chemical processes, compressible flow, FVM and turbulence [1]. Start by identifying the actual module (`XiFluid`, `multicomponentFluid`, `fluid` or `multiphaseEuler`), then run a short case and check species, thermodynamics, reaction rates, balances and stability before publishing it in the catalogue.
