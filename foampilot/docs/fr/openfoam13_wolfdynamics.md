...
Porter d’abord la cavité entraînée, Hagen–Poiseuille, le cylindre instationnaire et le dam-break. Chaque cas doit être généré par FoamPilot, être inspectable avant exécution et fournir un rapport compact : fichiers présents, maillage, résidus, conservation, grandeur physique de référence et résultat attendu avec tolérance.
...
L’édition Figshare d’octobre 2025 annonce explicitement OpenFOAM 13 et fournit des exemples ainsi qu’un support consacré aux processus chimiques, aux écoulements compressibles, à la FVM et à la turbulence [1]. Le portage doit commencer par l’inventaire des cas et la détection du module réellement utilisé (`XiFluid`, `multicomponentFluid`, `fluid` ou `multiphaseEuler`). Il faut ensuite exécuter un cas court, vérifier les espèces, la thermodynamique, les taux de réaction, les bilans et la stabilité avant de l’ajouter au catalogue public.
