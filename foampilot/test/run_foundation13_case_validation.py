"""Validate generated Foundation 13 cases without machine-local paths."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=REPO_ROOT,
        help="repository root used for the default cases",
    )
    parser.add_argument(
        "--case",
        action="append",
        default=[],
        metavar="NAME=PATH",
        help="additional or replacement case; may be repeated",
    )
    args = parser.parse_args()
    root = args.root.expanduser().resolve()
    cases = {
        "dtc_background": root / "openfoam13/DTCMoving_Overset_Foundation13/background",
        "foundation_propeller": root / "openfoam13/FoamPilotCases/propellerFoundation13",
    }
    for specification in args.case:
        name, path = parse_case(specification)
        cases[name] = path if path.is_absolute() else root / path
