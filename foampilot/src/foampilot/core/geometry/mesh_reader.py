...
def make_path(path, time_name=None, name=None):
    """Construct the full path to the OpenFOAM file."""
    if time_name is not None and name is None:
        path = Path(path) / time_name
    elif name is not None and time_name is None:
        path = Path(path) / name
    elif name is not None and time_name is not None:
        path = Path(path) / time_name / name
    if not path.exists() and path.with_suffix(".gz").exists():
        path = path.with_suffix(".gz")
    return path

def find_latest_time(path):
    """Find the latest time directory in the OpenFOAM case."""
    dir_list = os.listdir(path)
    time_list = [d for d in dir_list if d.replace('.', '', 1).isdigit()]
...
class OpenFoamFile:
    ...
    def __init__(self, path, time_name=None, name=None, structured=False, boundary=None, sets=None, order="F", precision=15, datatype=None, verbose=True):
        self.pathcase = Path(path)
        if time_name == "latestTime":
            time_name = find_latest_time(path)
        if sets is None:
            self.path = make_path(path, time_name, name)
        else:
            self.path = make_path(path, time_name, Path(sets) / name)
        self.verbose = verbose
        if self.verbose:
            print("Reading file " + str(self.path))

        self.is_compressed = self.path.suffix == ".gz"
        self._open = gzip.open if self.is_compressed else open

        with self._open(self.path, "rb") as f:
            self.content = f.read()

        self.lines_stripped = [line.strip().replace(b'"', b"").replace(b";", b"") for line in self.content.split(b"\n")]
...
def typefield(path, time_name=None, name=None, verbose=True):
    ...
    path = make_path(path, time_name, name)
    ...
    if boundary is not None:
        ...
        if (time_name is None and region is None
            ...
            nmesh = np.size(xs)
              ...
              and os.path.exists(make_path(path, time_name, "C"))):
            xs, ys, zs = readvector(
...
def getVolumes(
    ...
    if box is not None:
        ...
        if len(box[0]) != 3 or len(box[1]) != 3:
            raise ValueError("box mins and maxs must be float tuples of length 3")
    else:
