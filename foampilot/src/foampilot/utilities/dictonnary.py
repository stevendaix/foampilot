...
class OpenFOAMDictAddFile:
    ...
    def write_raw(self, name_dict, base_path, content, folder="system"):
        """Write an exact OpenFOAM dictionary body through FoamPilot.

        This is intentionally an escape hatch for dictionaries whose syntax
        cannot be represented by the legacy attribute serializer (for example
        directive blocks, named lists, or solver-specific dictionaries). The
        standard FoamFile header remains generated by FoamPilot.
        """
        path = Path(base_path) / folder
        path.mkdir(parents=True, exist_ok=True)
        filepath = path / name_dict
        with filepath.open("w", encoding="utf-8") as file:
            if "FoamFile" in content[:2000]:
