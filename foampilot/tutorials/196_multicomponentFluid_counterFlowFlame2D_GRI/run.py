"""OpenFOAM 13 multicomponentFluid/counterFlowFlame2D_GRI via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/multicomponentFluid/counterFlowFlame2D_GRI")
...
def run_parallel(solver: Solver, executable: str, *args: str, tag: str) -> None:
    command_line = shlex.join(["/usr/bin/mpirun", "--oversubscribe", "-np", "12", str(OF13_BIN / executable), "-parallel", *args])
    solver.run_command(["/bin/bash", "-lc", f"source /opt/openfoam13/etc/bashrc >/dev/null 2>&1 && exec {command_line}"], log_filename=f"log.{tag}", environment=OF13_ENV)
...
def main() -> None:
    ...
    run_of13(solver, "decomposePar", "-cellProc", tag="decomposePar.cellProc")
    run_parallel(solver, "foamRun", tag="foamRun.parallel")
    run_of13(solver, "reconstructPar", "-cellProc", tag="reconstructPar.cellProc")
