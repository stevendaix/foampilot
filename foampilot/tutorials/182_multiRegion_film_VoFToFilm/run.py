"""OpenFOAM 13 multiRegion/film/VoFToFilm via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/multiRegion/film/VoFToFilm")
...
def main() -> None:
    ...
    command(solver, "blockMesh", "-region", "VoF", tag="blockMesh.VoF")
    command(
        solver,
        "extrudeToRegionMesh",
        "-region",
        "VoF",
        tag="extrudeToRegionMesh.VoF",
    )
    command(solver, "setFields", "-region", "VoF", tag="setFields.VoF")
    solver.run_command(
