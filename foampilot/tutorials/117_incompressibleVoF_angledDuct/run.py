"""OpenFOAM 13 incompressibleVoF/angledDuct through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/incompressibleVoF/angledDuct")
RESOURCE = Path("/opt/openfoam13/tutorials/resources/blockMesh/angledDuct")
    ...
    for source in REFERENCE.rglob("*"):
        ...
        if relative.parts and relative.parts[0] == "0":
            solver.fields_manager.import_reference_field(
                source, destination, field_name=relative.relative_to("0")
            )
        else:
...
def main() -> None:
    ...
    solver.import_reference_asset(RESOURCE, case_path / "system" / "angledDuct")
    solver.run_command(
        ["blockMesh", "-dict", "system/angledDuct"], log_filename="log.blockMesh"
    )
