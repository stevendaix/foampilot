"""OpenFOAM 13 incompressibleVoF/wave3D through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/incompressibleVoF/wave3D")
...
def import_reference_case(solver: Solver, case_path: Path) -> None:
    """Import the complete OF13 wave3D case through FoamPilot APIs."""
...
def main() -> None:
    ...
    for name in ("blockMesh", "refineMesh", "setWaves"):
        solver.run_command(
            [str(OF13_BIN / name)], log_filename=f"log.{name}"
        )
    solver.run_command(
    ...
    solver.run_command(
        [str(OF13_BIN / "reconstructPar"), "-newTimes"],
