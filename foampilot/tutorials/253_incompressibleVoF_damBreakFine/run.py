"""OpenFOAM 13 incompressibleVoF/damBreakFine through FoamPilot only."""
...
def main() -> None:
    ...
    # damBreakFine overlays only the refined block, time step and decomposition.
    solver.system.merge_reference_dictionary(
        "system/blockMeshDict", REFERENCE / "system/blockMeshDict.orig",
        blocks=["blocks"],
    )
    solver.system.update_dictionary_entries(
        "system/controlDict", {"deltaT": "0.0005"}
    )
    import_tree(solver, REFERENCE, case_path)
    solver.run_command(["blockMesh"], log_filename="log.blockMesh")
    solver.run_command(["setFields"], log_filename="log.setFields")
    solver.run_simulation(nb_proc=4, log_filename="log.incompressibleVoF")
