"""OpenFOAM 13 solidDisplacement/plateHole via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/solidDisplacement/plateHole")
...
def main() -> None:
    ...
    solver.solver_name = "solidDisplacement"
    solver.transient = True
    solver.is_solid = True
    solver.setup_case()
    import_reference_case(solver, case_path)
    run_of13(solver, "blockMesh", tag="blockMesh")
    run_of13(solver, "foamRun", tag="foamRun")
    run_of13(solver, "foamPostProcess", "-func", "components(sigma)", tag="foamPostProcess.sigma")
    run_of13(solver, "foamPostProcess", "-func", "graphUniform", tag="foamPostProcess.graphUniform")
