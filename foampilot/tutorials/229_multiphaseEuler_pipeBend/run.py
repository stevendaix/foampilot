"""OpenFOAM 13 multiphaseEuler/pipeBend via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/multiphaseEuler/pipeBend")
...
def run_parallel(solver: Solver, executable: str, *args: str, tag: str) -> None:
    command_line = shlex.join(["/usr/bin/mpirun", "--oversubscribe", "-np", "6", str(OF13_BIN / executable), "-parallel", *args])
    solver.run_command(
        ["/bin/bash", "-lc", f"source /opt/openfoam13/etc/bashrc >/dev/null 2>&1 && exec {command_line}"],
        log_filename=f"log.{tag}",
        environment=OF13_ENV,
...
def main() -> None:
    ...
    run_parallel(
        ...
        "multiphaseEuler",
        "-func",
        "populationBalanceSetPhaseSizeDistribution",
        tag="populationBalanceSetPhaseSizeDistribution.parallel",
    )
    ...
    run_of13(
        ...
        "multiphaseEuler",
        "-func",
        "graphLayerAverage(name=graphCrossSection,patches=(inlet),axis=distance,d.particles)",
        "-latestTime",
        tag="graphLayerAverage.latestTime",
    )
