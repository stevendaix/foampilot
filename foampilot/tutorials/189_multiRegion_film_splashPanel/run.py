"""OpenFOAM 13 multiRegion/film/splashPanel via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/multiRegion/film/splashPanel")
...
def run_of13(solver: Solver, executable: str, *args: str, tag: str) -> None:
    executable_path = OF13_TOOLS / executable if executable == "paraFoam" else OF13_BIN / executable
    command_line = shlex.join([str(executable_path), *args])
    solver.run_command(
...
def main() -> None:
    ...
    run_of13(solver, "blockMesh", "-region", "fluid", tag="blockMesh.fluid")
    run_of13(solver, "extrudeToRegionMesh", "-region", "fluid", tag="extrudeToRegionMesh.film")
    run_of13(solver, "paraFoam", "-touchAll", tag="paraFoam.touchAll")
    run_of13(solver, "foamMultiRun", tag="foamMultiRun")
