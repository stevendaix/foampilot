"""OpenFOAM 13 potentialFoam/pitzDaily via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/potentialFoam/pitzDaily")
...
def main() -> None:
    ...
    solver.solver_name = "potentialFoam"
    solver.transient = True
    solver.setup_case()
    import_reference_case(solver, case_path)
    solver.import_reference_asset(
        Path("/opt/openfoam13/tutorials/resources/blockMesh/pitzDaily"),
        case_path / "system/pitzDaily",
    )
    run_of13(solver, "blockMesh", "-dict", "system/pitzDaily", tag="blockMesh")
    run_of13(solver, "potentialFoam", "-writePhi", "-writep", tag="potentialFoam")
    run_of13(solver, "foamPostProcess", "-func", "streamFunction", tag="foamPostProcess.streamFunction")
