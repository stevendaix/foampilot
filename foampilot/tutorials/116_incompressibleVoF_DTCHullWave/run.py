"""OpenFOAM 13 incompressibleVoF/DTCHullWave through FoamPilot only."""
...
def import_fields(solver: Solver, source_root: Path, destination: Path) -> None:
    for source in (source_root / "0").glob("*.orig"):
        solver.fields_manager.import_reference_field(
            source, destination, field_name=source.stem
        )


def main() -> None:
    root = Path.cwd()
    mesh_case = root / "meshSource"
    case = root / "DTCHullWave"
    ...
    solver.setup_case()
    import_tree(mesh_solver, REFERENCE, mesh_case)
    import_tree(solver, REFERENCE, case)
    import_fields(solver, REFERENCE, case)
    ...
    solver.run_command(["setWaves"], log_filename="log.setWaves")
