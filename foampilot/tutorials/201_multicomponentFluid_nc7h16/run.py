"""OpenFOAM 13 multicomponentFluid/nc7h16 via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/multicomponentFluid/nc7h16")
...
def import_reference_case(solver: Solver, case_path: Path) -> None:
    ...
    for root in ("constant", "system"):
        if root == "constant":
            chemkin_root = Path("/opt/openfoam13/test/chemistry/nc7h16/chemkin")
            for source in chemkin_root.rglob("*"):
                if source.is_file():
                    solver.import_reference_asset(source, case_path / "chemkin" / source.name)
...
def main() -> None:
    ...
    run_of13(solver, "zeroDimensionalMesh", tag="zeroDimensionalMesh")
    run_of13(solver, "chemkinToFoam", "chemkin/chem.inp", "chemkin/therm.dat", "chemkin/transportProperties", "constant/reactions", "constant/speciesThermo", tag="chemkinToFoam")
    run_of13(solver, "foamPostProcess", "-func", "massFractions", tag="foamPostProcess.massFractions")
