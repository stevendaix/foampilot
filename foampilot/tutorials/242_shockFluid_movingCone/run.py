"""OpenFOAM 13 shockFluid/movingCone via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/shockFluid/movingCone")
...
def main() -> None:
    ...
    for map_time in ("1e-05", "2e-05"):
        solver.import_reference_asset(
            REFERENCE / "system/meshes" / map_time / "decomposeParDict",
            case_path / "system/decomposeParDict",
        )
        run_of13(solver, "blockMesh", "-mesh", map_time, tag=f"blockMesh.{map_time}")
        run_of13(solver, "decomposePar", "-mesh", map_time, tag=f"decomposePar.{map_time}")
