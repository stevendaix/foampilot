"""OpenFOAM 13 shockFluid/biconic25-55Run35 via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/shockFluid/biconic25-55Run35")
...
def main() -> None:
    ...
    solver.import_reference_asset(
        REFERENCE / "constant/grid256.dat",
        case_path / "constant/grid256.dat",
    )
    run_of13(solver, "blockMesh", tag="blockMesh")
    run_of13(solver, "datToFoam", "constant/grid256.dat", tag="datToFoam")
    solver.merge_mesh_points("constant/points.tmp")
    run_of13(solver, "collapseEdges", "-dict", "system/collapseDict", tag="collapseEdges")
    solver.update_mesh_patch_types({"wedge1": "wedge", "wedge2": "wedge"})
