"""OpenFOAM 13 multiphaseEuler/aeratedStirredTankMRF via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/multiphaseEuler/aeratedStirredTankMRF")
...
def import_reference_case(solver: Solver, case_path: Path) -> None:
    for source in (REFERENCE / "0").rglob("*"):
        if source.is_file():
            ...
            solver.fields_manager.import_reference_field(source, case_path, field_name=str(relative))
...
def run_parallel(solver: Solver, executable: str, *args: str, tag: str) -> None:
    command_line = shlex.join(["/usr/bin/mpirun", "--oversubscribe", "-np", "8", str(OF13_BIN / executable), "-parallel", *args])
    solver.run_command(
        ["/bin/bash", "-lc", f"source /opt/openfoam13/etc/bashrc >/dev/null 2>&1 && exec {command_line}"],
        log_filename=f"log.{tag}",
        environment=OF13_ENV,
...
def main() -> None:
    ...
    run_of13(
        ...
        "-baffles",
        "baffles",
        tag="snappyHexMeshConfig",
    )
    run_of13(solver, "blockMesh", tag="blockMesh")
    run_of13(solver, "decomposePar", "-copyZero", tag="decomposePar.copyZero")
    run_parallel(solver, "snappyHexMesh", tag="snappyHexMesh.parallel")
    run_parallel(solver, "setFields", tag="setFields.parallel")
    run_of13(
        solver,
        "reconstructPar",
        "-withZero",
        "-fields",
        "(alpha.gas alpha.liquid)",
        tag="reconstructPar.fractions",
    )
    run_parallel(solver, "foamRun", tag="foamRun.parallel")
    run_of13(solver, "reconstructPar", "-a", tag="reconstructPar.latest")
