"""OpenFOAM 13 legacy mdFoam/nanoNozzle via FoamPilot."""
...
REFERENCE = Path(
    "/opt/openfoam13/tutorials/legacy/lagrangian/mdFoam/nanoNozzle"
)
...
def import_reference_case(solver: Solver, case_path: Path) -> None:
    """Import the complete OF13 molecular-dynamics nozzle case."""
...
def main() -> None:
    ...
    solver.run_command(
        mpi_command("mdInitialise"), log_filename="log.mdInitialise.parallel"
    )
    solver.run_command(
        mpi_command("mdFoam"), log_filename="log.mdFoam.parallel"
    )
    solver.run_command(
