"""OpenFOAM 13 fluid/iglooWithFridges through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/fluid/iglooWithFridges")
...
def main() -> None:
    ...
    for source in (REFERENCE / "constant").iterdir():
        if source.is_file() and source.name != "geometry":
            solver.constant.import_reference_file(source)
    for source in (REFERENCE / "constant" / "geometry").iterdir():
        if source.is_file():
            solver.import_reference_asset(source, Path("constant/geometry") / source.name)
    ...
    solver.system.run_utility("snappyHexMesh", ["-overwrite"], log_filename="log.snappyHexMesh")
