...
Les garde-fous de configuration rejettent maintenant les clouds vides, les fractions négatives et les fractions dont la somme diffère de un. Le test `test_negative_audit.py` vérifie que l’auditeur rejette une double création de même clé et une confirmation orpheline. La détection des confirmations en double reste également rejetée par la réconciliation MPI. Un test d’intégration incompressible et une validation explicite du comportement d’un nom de cloud inconnu restent recommandés avant le merge de production.
