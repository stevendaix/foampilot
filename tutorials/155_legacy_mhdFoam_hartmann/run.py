"""OpenFOAM 13 legacy mhdFoam/hartmann via FoamPilot."""
...
REFERENCE = Path(
    "/opt/openfoam13/tutorials/legacy/electromagnetics/mhdFoam/hartmann"
)
...
def main() -> None:
    ...
    solver.solver_name = "mhdFoam"
    solver.transient = True
    solver.setup_case()
    import_reference_case(solver, case_path)

    solver.run_command([str(OF13_BIN / "blockMesh")], log_filename="log.blockMesh")
    solver.run_command([str(OF13_BIN / "mhdFoam")], log_filename="log.mhdFoam")
    solver.run_command(
        [str(OF13_BIN / "foamPostProcess"), "-func", "sample"],
        log_filename="log.foamPostProcess.sample",
    )
