"""OpenFOAM 13 incompressibleVoF/climbingRod through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/incompressibleVoF/climbingRod")


def import_reference_case(solver: Solver, destination: Path) -> None:
    for source in REFERENCE.rglob("*"):
        if not source.is_file() or source.name in {"Allrun", "Allclean", "README"}:
            continue
