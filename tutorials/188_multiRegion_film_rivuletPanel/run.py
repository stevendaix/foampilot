"""OpenFOAM 13 multiRegion/film/rivuletPanel via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/multiRegion/film/rivuletPanel")
...
OF13_ENV = {
    ...
    "LD_LIBRARY_PATH": f"{OF13_TP_MPI_LIB}:{OF13_MPI_LIB}:{OF13_PV_LIB}:/usr/lib/x86_64-linux-gnu:/usr/lib/x86_64-linux-gnu/openmpi/lib:{OF13_TP_LIB}:{OF13_DUMMY_LIB}:{OF13_LIB}:{os.environ.get('LD_LIBRARY_PATH', '')}",
}
...
def parallel_command(solver: Solver, executable: str, *args: str, tag: str) -> None:
    command_line = shlex.join(
        ["/usr/bin/mpirun", "--oversubscribe", "-np", str(NPROCS),
         str(OF13_BIN / executable), "-parallel", *args]
    )
    solver.run_command(
        ["/bin/bash", "-lc", f"source /opt/openfoam13/etc/bashrc >/dev/null 2>&1 && exec {command_line}"],
        log_filename=f"log.{tag}",
...
def main() -> None:
    ...
    command(solver, "blockMesh", "-region", "panel", tag="blockMesh.panel")
    command(
        solver,
        "decomposePar",
        "-region",
        "panel",
        "-noFields",
        tag="decomposePar.panel.noFields",
    )
    parallel_command(
        solver,
        "extrudeToRegionMesh",
    ...
    command(
        ...
        "-fields",
        "-allRegions",
        tag="decomposePar.fields.allRegions",
    )
    solver.run_command(
        ["/bin/bash", "-lc", "source /opt/openfoam13/etc/bashrc >/dev/null 2>&1 && exec " + shlex.join([str(OF13_TOOLS / "paraFoam"), "-touchAll"])],
