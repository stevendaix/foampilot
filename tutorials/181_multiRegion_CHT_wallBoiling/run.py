"""OpenFOAM 13 multiRegion/CHT/wallBoiling via FoamPilot."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/multiRegion/CHT/wallBoiling")
...
def main() -> None:
    ...
    command(solver, "extrudeMesh", tag="extrudeMesh")
    command(
        solver,
        "splitMeshRegions",
        "-cellZones",
    ...
    solver.run_command(
        [str(OF13_TOOLS / "paraFoam"), "-region", "fluid", "-touch"],
        log_filename="log.paraFoam.fluid.touch",
    )
    solver.run_command(
    ...
    command(
        ...
        "-latestTime",
        "-allRegions",
        tag="reconstructPar.latestTime.allRegions",
    )
    command(
        solver,
        "foamPostProcess",
        ...
        "fluid",
        "-func",
        "graphCell(name=graph,start=(3.4901 0 0),end=(3.4901 0.0096 0),fields=(alpha.gas T.liquid T.gas))",
        tag="foamPostProcess.graphCell",
    )
    command(
        solver,
        "foamPostProcess",
        ...
        "-latestTime",
        "-func",
        "patchSurface(name=patchWallBoilingProperties,patch=wall,surfaceFormat=raw,interpolate=false,fields=(wallBoiling:dDeparture wallBoiling:fDeparture wallBoiling:nucleationSiteDensity wallBoiling:wetFraction))",
        tag="foamPostProcess.patchSurface",
    )
