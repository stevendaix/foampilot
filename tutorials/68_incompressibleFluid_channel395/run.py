"""OpenFOAM 13 incompressibleFluid/channel395 through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/incompressibleFluid/channel395")
...
def main() -> None:
    ...
    for source in (REFERENCE / "0").rglob("*"):
        if source.is_file():
            field_name = source.name[:-3] if source.name.endswith(".gz") else source.name
    ...
    solver.run_parallel(nb_proc=4, log_filename="log.incompressibleFluid", force_decompose=True)
