"""OpenFOAM 13 mesh/spiralPipe through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/mesh/spiralPipe")
...
def import_reference_case(solver: Solver, case_path: Path) -> None:
    ...
    solver.import_reference_asset(
        REFERENCE / "constant/geometry/curve.obj.gz",
        case_path / "constant/geometry/curve.obj",
    )


def run_of13(solver: Solver, executable: str, *, tag: str) -> None:
    solver.run_command(
...
def main() -> None:
    ...
    solver.solver_name = "extrudeMesh"
    solver.setup_case()
