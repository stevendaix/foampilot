"""OpenFOAM 13 incompressibleDriftFlux/tank3D through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/incompressibleDriftFlux/tank3D")


def main() -> None:
    case_path = Path.cwd()
    solver = Solver(case_path)
    solver.solver_name = "incompressibleDriftFlux"
    ...
    for source in (REFERENCE / "constant").rglob("*"):
        if source.is_file():
            ...
            solver.import_reference_asset(source, Path("constant") / relative_name)
    ...
    solver.run_simulation(nb_proc=1, log_filename="log.incompressibleDriftFlux")
