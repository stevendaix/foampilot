"""OpenFOAM 13 incompressibleFluid/moodyChart through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/incompressibleFluid/moodyChart")
...
def main() -> None:
    ...
    for source in (REFERENCE / "system").rglob("*"):
        if source.is_file():
            ...
            target = rel.parent / active_name(source)
            solver.system.import_reference_file(source, filename=target)
    for source in (REFERENCE / "constant").rglob("*"):
        if source.is_file():
            rel = source.relative_to(REFERENCE / "constant")
            target = rel.parent / active_name(source)
            solver.constant.import_reference_file(source, filename=target)
