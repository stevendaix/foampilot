"""OpenFOAM 13 incompressibleFluid/elipsekkLOmega through FoamPilot only."""
...
REFERENCE = Path("/opt/openfoam13/tutorials/incompressibleFluid/elipsekkLOmega")
...
def main() -> None:
    ...
    solver.system.run_utility(
        "transformPoints", args=["scale=(1.6666 1 1)"], log_filename="log.transformPoints"
    )
    solver.system.run_utility(
        "mirrorMesh", args=["-dict", "mirrorMeshDict.x"], log_filename="log.mirrorMesh.x"
    )
    solver.system.run_utility(
        "mirrorMesh", args=["-dict", "mirrorMeshDict.y"], log_filename="log.mirrorMesh.y"
    )
