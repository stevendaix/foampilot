...
| `237_shockFluid_LadenburgJet60psi` | Présent | Présent | Conforme OF13 | Validé — cas compressible `shockFluid`, champs `T/U/p`, 8 domaines MPI, pas adaptatif avec `maxCo=0,5`, calcul jusqu’à `End=2e-5 s` et reconstruction terminée sans `FOAM FATAL` |
| `238_shockFluid_biconic25-55Run35` | Présent | Présent | Conforme OF13 | Accepté avec réserves — `datToFoam`, fusion FoamPilot de `points.tmp`, `collapseEdges`, conversion `wedge1/wedge2` et calcul `shockFluid` terminés; progression jusqu’à `Time≈4,90e-4 s` sur `1e-3 s` sans `FOAM FATAL`, avec réserve de temps et avertissement de planéité possible de `wedge2` |
...
| `247_solidDisplacement_square` | Absent de la référence OF13 | Aucun runner | Non applicable | Le chemin `/opt/openfoam13/tutorials/solidDisplacement/square` n’existe pas dans OpenFOAM 13; seuls `beamEndLoad` et `plateHole` sont distribués dans cette famille. Voir `docs/audit_openfoam13_solidDisplacement_square.md` |
| `248_XiFluid_1D` | Présent | Présent | Conforme OF13 | Validé — cas XiFluid 1D découvert lors de la comparaison complète de la distribution OF13; import des champs de combustion, `blockMesh`, `setFields` et `foamRun` jusqu’à `End=0,2 s`, Courant maximal proche de 0,38, erreur cumulative de continuité de l’ordre de `5e-13`, aucun `FOAM FATAL` ni NaN |
| `249_XiFluid_kivaTest` | Présent | Présent | Conforme OF13 | Accepté avec réserve — cas moteur `XiFluid` découvert dans l’inventaire OF13, import de `otape17` et du temps utilisateur `-180/`, conversion `kivaToFoam`, calcul jusqu’à `42,575/60 CAD` au plafond de 300 s, Courant maximal proche de 0,029, aucune erreur fatale visible |
| `250_incompressibleFluid_pitzDailySteadyMappedToRefined` | Présent | Présent | Conforme OF13 — validation partielle | Cas multi-maillages source/cible préparé avec imports FoamPilot, raffinement de `blockMeshDict`, décomposition et mapping prévus; le cas source atteint `End=300 s` sans `FOAM FATAL`, mais le plafond de 300 s intervient avant l’exécution cible/mapping/reconstruction |
| `251_incompressibleFluid_wingMotion` | Présent | Présent | Conforme OF13 | Accepté avec réserve — chaîne parent complète `blockMesh → snappyHexMesh → extrudeMesh → createPatch → foamRun stationnaire → mapFields → decomposePar → foamRun mobile`; préparation et mapping validés, calcul transitoire stable jusqu’à `0,2076/5 s`, reconstruction finale hors budget, aucun `FOAM FATAL` |
| `252_incompressibleFluid_wingMotion_wingMotion_snappyHexMesh` | Présent | Couvert par `251_incompressibleFluid_wingMotion` | Conforme OF13 | Sous-cas de préparation `blockMesh`/`snappyHexMesh` exécuté dans le runner parent; aucun runner séparé afin d’éviter un doublon |
| `253_incompressibleVoF_damBreakFine` | Présent | Présent | Conforme OF13 | Validé — `blockMesh`, `setFields`, décomposition 4 domaines, `foamRun` parallèle et reconstruction jusqu’à `Time=1 s`; `Courant max≈0,878`, avertissements non bloquants, aucun `FOAM FATAL` |
| `254_incompressibleVoF_damBreakPorousBaffle` | Présent | Présent | Conforme OF13 | Validé — `blockMesh`, `setFields`, `createBaffles` (13 faces, patches `porous_half0/1`) et `foamRun` jusqu’à `Time=1 s`; Courant max≈0,194, aucun `FOAM FATAL` |
| `255_isothermalFluid_potentialFreeSurfaceMovingOscillatingBox` | Présent | Présent | Conforme OF13 | Validé — `blockMesh`, `subsetMesh`, `foamRun` mobile jusqu’à `Time=20 s`; champs `pointDisplacement/cellDisplacement` produits, aucun `FOAM FATAL` |
