...
| 5 | `XiFluid` | `engine2Valve2D` | `/opt/openfoam13/tutorials/XiFluid/engine2Valve2D` | — | À traiter | — | — |
...
| 87 | `incompressibleFluid` | `pitzDailyScalarTransport` | `/opt/openfoam13/tutorials/incompressibleFluid/pitzDailyScalarTransport` | `05_scalarTransport` | En cours — API validée | `Functions.scalar_transport`, `Functions.write_function_object`, `CaseFieldsManager.register_field` avec valeurs `uniform`/`nonuniform` | Le function object `scalarTransport` est généré par Foampilot et le runner s’exécute sans erreur ; la géométrie et les champs doivent encore être réalignés sur la référence `pitzDailyScalarTransport` (maillage officiel et champs initiaux issus de pitzDaily). |
